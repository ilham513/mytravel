		  ...
		  <section class="mb-4">
			<div class="card">
			  <div class="card-header py-3">
				<h5 class="mb-0"><strong>Ubah Booking</strong></h5>
			  </div>
			  <div class="card-body">
				<form action="<?=site_url('/admin/booking_edit_go')?>" method="post">
					<div class="form-group mb-2">
						<input name="id_booking" type="hidden" class="form-control" value="<?=$obj_booking->id_booking?>">
					</div>
					<div class="form-group mb-2">
					  <label for="nama">Pilih Tujuan:</label>
						<select name="id_tujuan" class="form-select form-select-lg mb-3" aria-label=".form-select-lg example">
							...
							<option <?=$tujuan->id_tujuan == $obj_booking->id_tujuan ? 'selected' : '';?> value="<?=$tujuan->id_tujuan?>"><?=$tujuan->nama_tujuan?></option>
							<?php endforeach; ?>
						</select>
					</div>
					<div class="form-group mb-2">
						...
						<select name="id_pelanggan" class="form-select form-select-lg mb-3" aria-label=".form-select-lg example">
							...
							<option <?=$pelanggan->id_pelanggan == $obj_booking->id_pelanggan ? 'selected' : '';?> value="<?=$pelanggan->id_pelanggan?>"><?=$pelanggan->nama_pelanggan?></option>
							<?php endforeach; ?>
						</select>
					</div>
					<div class="form-group mb-2">
					  <label for="nama">Tanggal Keberangkatan:</label>
					  <input name="tanggal_keberangkatan" type="date" class="form-control" value="<?=$obj_booking->tanggal_keberangkatan?>" id="nama">
					</div>
					<div class="form-group mb-2">
					  <label for="nama">Armada:</label>
						<select name="id_armada" class="form-select form-select-lg mb-3" aria-label=".form-select-lg example">
							...
							<option <?=$armada->id_armada == $obj_booking->id_armada ? 'selected' : '';?> value="<?=$armada->id_armada?>"><?=$armada->nama_armada?></option>
							<?php endforeach; ?>
						</select>
					</div>
